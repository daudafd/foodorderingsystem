...
$soups = null;
if ($qry['category_id'] == 10) {
    $soups = $conn->query("SELECT * FROM soup_options");
}

// Check if the product has size options (meat options)
$hasSizeOptions = $conn->query("SELECT 1 FROM meat_options WHERE meat_type = '" . $qry['name'] . "' LIMIT 1")->fetch_row();

// Fetch size options with prices
$sizeOptions = [];
if ($hasSizeOptions) {
    $sizeResults = $conn->query("SELECT size, price FROM meat_options WHERE meat_type = '" . $qry['name'] . "'");
    if ($sizeResults && $sizeResults->num_rows > 0) {
        while ($row = $sizeResults->fetch_assoc()) {
...
<div class="container-fluid">
    <div class="card">
        ...
        <div class="card-body">
            ...
            <p class="card-text truncate"><?php echo htmlspecialchars($qry['description']); ?></p>
            ...
            <?php else: ?>
                <p class="card-text">#<?php echo number_format($qry['price'], 2); ?></p>
            <?php endif; ?>

            <?php if ($qry['category_id'] == 10 && $soups && $soups->num_rows > 0): ?>
                <div class="form-group">
                    <label>Choose Soup:</label>
                    <select id="soupSelect" class="form-control">
                        <?php while ($soup = $soups->fetch_assoc()): ?>
...
$(document).ready(function() {
    ...
    function updatePrice(){
        ...
        let productName = '<?php echo $qry['name'] ?>';
        $.ajax({
